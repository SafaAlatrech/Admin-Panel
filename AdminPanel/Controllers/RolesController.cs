...
namespace RoleBasedAuthorization.Controllers
{
    public class RolesController : Controller
    {
        ...
        public RolesController(MyDbContext db)
        {
            this.db = db;
        }
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var roles = await db.Roles.SingleOrDefaultAsync(m => m.Id == id);
            if (roles == null)
            {
                return NotFound();
            }
            ...
            List<string> menus_id = db.LinkRolesMenus.Where(s=> s.RolesId == id).Select(s => s.MenusId.ToString()).ToList();
            ds = ToDataSet(db.Menus.ToList());
        ...
        {
            foreach (var role in db.LinkRolesMenus.Where(s=>s.RolesId == id))
            {
                db.LinkRolesMenus.Remove(role);
            }
            await db.SaveChangesAsync();
        ...
        {
            var temp = db.LinkRolesMenus.Where(s => s.RolesId == id);
            ...
            {
                db.LinkRolesMenus.Add(new LinkRolesMenus { MenusId = role, RolesId = id });
            }
